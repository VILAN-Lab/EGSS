...
class Encoder(nn.Module):
    def __init__(self, embeddings, vocab_size, embedding_size, hidden_size, num_layers, dropout):
        ...
        lstm_input_size = embedding_size + 210
    ...
    def forward(self, src_seq, src_len, tag_seq, ner_seq, pos_seq):
        ...
        return outputs,  h #concat_states


class An_Encoder(nn.Module):
    def __init__(self, embeddings, vocab_size, embedding_size, hidden_size, num_layers, dropout):
        super(An_Encoder, self).__init__()
    ...
    def forward(self, an_seq, an_len):
        total_length = an_seq.size(1)
        embedded = self.embedding(an_seq)
...
class Seq2seq(nn.Module):
    def __init__(self, embedding=None):
        ...
        self.encoder = Encoder(embedding,
                               ...
                               config.num_layers,
                               config.dropout)
        self.an_encoder = An_Encoder(embedding,
                               config.vocab_size,
                               config.embedding_size,
                               config.hidden_size,
                               config.num_layers,
    ...
    def forward(self, src_seq, tag_seq, ext_src_seq, trg_seq, adj_seq, ner_seq, an_seq, pos_seq):
        enc_mask = torch.sign(src_seq)
        src_len = torch.sum(enc_mask, 1)
        an_enc_mask = torch.sign(an_seq)
        an_len = torch.sum(an_enc_mask, 1)

        enc_output1 = self.gcn(src_seq, adj_seq[0], tag_seq, ner_seq, pos_seq)

        an_enc_outputs, an_c = self.an_encoder(an_seq, an_len)

        enc_outputs, enc_h = self.encoder(src_seq, src_len, tag_seq, ner_seq, pos_seq)

        enc_states = an_c
        enc_outputs = torch.cat((enc_outputs, enc_output1), 2)
        enc_outputs = self.layer1(enc_outputs)
