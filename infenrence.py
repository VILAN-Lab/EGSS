...
class BeamSearcher(object):
    def __init__(self, model_path, output_dir):
        ...
        self.data_loader = get_loader(config.test_src_file,
                                      ...
                                      config.test_bio_file,
                                      config.test_adj_file,
                                      config.test_ner_file,
                                      config.test_an_file,
                                      config.test_pos_file,
                                      word2idx,
                                      batch_size=1,
                                      use_tag=True,
                                      shuffle=False)
        ...
        self.pred_dir = os.path.join(output_dir, "generated-03.02(wudaan).txt")
        self.golden_dir = os.path.join(output_dir, "golden25.txt")
        self.src_file = os.path.join(output_dir, "src25.txt")
    ...
    def decode(self):
        pred_fw = open(self.pred_dir, "w", encoding="utf-8")
        golden_fw = open(self.golden_dir, "w")
        for i, eval_data in enumerate(self.data_loader):
            src_seq, ext_src_seq, _, \
                _, tag_seq, oov_lst, adj_seq, ner_seq, an_seq, pos_seq = eval_data

            best_question = self.beam_search(src_seq, ext_src_seq, tag_seq, adj_seq, ner_seq, an_seq, pos_seq)
            # discard START  token
    ...
    def beam_search(self, src_seq, ext_src_seq, tag_seq, adj_seq, ner_seq, an_seq, pos_seq):
        enc_mask = torch.sign(src_seq)
        src_len = torch.sum(enc_mask, 1)
        an_enc_mask = torch.sign(an_seq)
        an_len = torch.sum(an_enc_mask, 1)
        ...
        if config.use_gpu:
            ...
            an_seq = an_seq.to(config.device)
            pos_seq = pos_seq.to(config.device)
            ner_seq = ner_seq.to(config.device)
            adj_seq = np.array([inst.numpy() for inst in adj_seq])
            adj_seq = torch.LongTensor(adj_seq)
            adj_seq = adj_seq.to(config.device)
        # forward encoder
        enc_outputs, enc_h = self.model.encoder(src_seq, src_len, tag_seq, ner_seq, pos_seq)

        an_enc_outputs, an_c = self.model.an_encoder(an_seq, an_len)
        # enc_states = (enc_h, an_c)
        enc_states = an_c
        enc_output1 = self.model.gcn(src_seq, adj_seq[0], tag_seq, ner_seq, pos_seq)
        enc_outputs = self.model.layer1(torch.cat((enc_outputs, enc_output1), 2))
        ...
        while num_steps < config.max_decode_step and len(results) < config.beam_size:
            ...
            prev_h = torch.stack(all_state_h, dim=1)
            prev_c = torch.stack(all_state_c, dim=1)
